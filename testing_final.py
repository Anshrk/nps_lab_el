...
def fetch_graph_data_from_neo4j(driver):
    """Extracts the graph structure from Neo4j."""
    ...
    with driver.session() as session:
        ...
        for record in results:
            ...
            if n.element_id not in nodes_dict:
                ...
                nodes_dict[n.element_id] = {"id": n.element_id, "label": n.get("name", n.get("ip", str(n.element_id))), "group": group, "scale_level": n.get("scale_level", 1)}
            
            if m.element_id not in nodes_dict:
                group = list(m.labels)[0].lower() if m.labels else "endpoint"
                nodes_dict[m.element_id] = {"id": m.element_id, "label": m.get("name", m.get("ip", str(m.element_id))), "group": group, "scale_level": m.get("scale_level", 1)}
            
            edges.append({
                "from": n.element_id, "to": m.element_id,
                "title": f"Relation: {r.type} | Z-Score: {r.get('z_score', '0.0')}",
                "time_step": r.get("time_step", 1), "scale_level": r.get("scale_level", 1),
                "is_threat": r.get("is_threat", False), "z_score": r.get("z_score", "0.0")
            })
